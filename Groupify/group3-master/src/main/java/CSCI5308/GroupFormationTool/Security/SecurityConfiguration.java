...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	public void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests().antMatchers("/public/**", "/**").permitAll().antMatchers("/admin/**").hasRole("ADMIN")
				.antMatchers("/question/**").hasRole("Instructor").anyRequest().authenticated().and().formLogin()
	...
	protected AuthenticationManager authenticationManager() throws Exception {
		...
		return securityAbstractFactory.returnCustomAuthenticationManagerInstance();
	}
}
